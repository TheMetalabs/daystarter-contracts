...
contract PreSale is Ownable {
  ...
  uint256 public preSale1Height;
  // second preSale block number
  uint256 public preSale2Height;
  // publicSale block number
  uint256 public publicSaleHeight;
  // sale price
  uint256 public salePrice;

  // Pre-sale Account Info
  ...
  function setPresaleInfo(uint256 _preSale1Height, uint256 _preSale2Height, uint256 _publicSaleHeight ) public onlyOwner {
...
// [유저] ETH로 DST 구매
  function sale( ) public payable{
    ...
    require(price % salePrice == 0, "price error");

    uint blockNumber = block.number;
    uint256 amount = price / salePrice;
    if(blockNumber >= preSale1Height && blockNumber < preSale2Height )     {
      // 1차 판매중
    ...
    } else if(blockNumber >= preSale2Height && blockNumber < publicSaleHeight )     {
      // 2차판매
      ...
      IERC20(dstAddress).transfer(msg.sender, amount);
    } else if(blockNumber >= publicSaleHeight) {
      // 퍼블릭세일
      revert("not saled");
    } else {
      revert("not saled");
