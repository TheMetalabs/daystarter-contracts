...
contract("PrivateSaleVesting", async (accounts) => {
  ...
  describe("Total", () => {
    ...
    it("M+0", async () => {
      ...
      const diff = Math.floor(
        moment.duration(current.diff(started)).as("days")
      );
  ...
  describe("Total with claim", () => {
    ...
    it("M+0", async () => {
      ...
      let balance = await vestingInstance.getClaimableBalance(
        privateSaleVester
      );
      ...
      const vestingAccountInfo = await vestingInstance.getVestingAccountInfo(
        privateSaleVester
      );
      ...
      assert.equal(
        vestingAccountInfo.claimedBalance,
        initialRelease.toString()
      );
    ...
    it("Cliff: M+1 => M+5", async () => {
      for (let i = 1; i <= 5; i++) {
        ...
        const balance = await vestingInstance.getClaimableBalance(
          privateSaleVester
        );
    ...
    it("Periodic: M+6 => M+28", async () => {
      for (let i = 6; i <= 28; i++) {
        ...
        await vestingInstance.claim({ from: privateSaleVester });
        const vestingInfo = await vestingInstance.getVestingInfo(
          privateSaleType
        );
        const vestingAccountInfo = await vestingInstance.getVestingAccountInfo(
          privateSaleVester
        ...
        block = await web3.eth.getBlock("latest");
        assert.equal(
          prevDstBalance.plus(periodicRelease).toString(),
          dstBalance.toString()
        );
  ...
  describe("Individual with claim", () => {
    const total = BigNumber("1000").times(decimal);
    const initialRelease = BigNumber(total).times(0.05);
    const periodicRelease = BigNumber(total).times(0.04);
